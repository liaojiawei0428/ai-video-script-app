...
>
> **📌 职责划分 (S72 起)**:
> - **本文件 (DEV_PROGRESS.md)**: 只维护 S18 之前的 GAP 清单 + 已完成模块状态 (§ 总体目标回顾 + § 已知 GAP 清单 + § 验证阶段 + § 风险与缓解 + § 安全轮换清单)
> - **AI 会话追踪**: 已转移到 [`HANDOVER.md` § 2.1](HANDOVER.md) (从 S64 起, S64-S72 都在那里)
> - **新 session 开始**: 必读 [`HANDOVER.md` § 0 30 秒速览](HANDOVER.md) (项目当前状态) + 本文件 (历史 GAP) + [`AGENTS.md`](AGENTS.md) (跨端规范)
...
| S81 | 2026-07-06 | **[已部署] v3.0.97 BUG-172 强制升级 9 维 E2E 闭环 + 实战演练** — commit b4a73af. 装老版 v3.0.95 APK(versionCode 96) → 启动 checkForUpdate success → server latestVersion=3.0.96 触发 modal → ForceUpdateModal 完整渲染 (335KB 截图 vs 黑屏 17K) → tap "立即升级 v3.0.96" (坐标 540,1612) → Chrome 跳转 ab.maque.uno/app/DeepScript_v3.0.96.apk → 下载 30324752 bytes → pm install -r v3.0.96 (versionCode 97) → 启动 v3.0.96 modal 消失进登录页 "Deep剧本 v3.0.96" (9 维 E2E 全过). **跨项目通用铁律 #31** (修 BUG 必跑端到端 E2E, logcat/console success ≠ UI 渲染, 必截图 + UI 树双重验证, 跟 BUG-079 假报告 100% 同源) + **#32** (4 状态机组件实例必 useState 同步, 不用 module-level state, 跟 BUG-138 polling owner 1:1 镜像). 跟 BUG-165/166 强制升级铁律闭环. AGENTS.md § 4.16 累计 13 条跨项目通用铁律配套沉淀 | v3.0.98 APK 签名 / 强制升级 modal UI 重设计 | 
| S82 | 2026-07-06 | **[已部署] v3.0.98 强制升级 modal UI 重设计** — commit 963a02f. iOS 26 液态玻璃 + Material 3 商业级设计: 删红色/警告/淘汰废话文案, 加 APP 内下载主推路径 + 内嵌下载进度条 (跟 v3.0.96 Chrome 跳转外链的笨重路径对比). 跨端铁律 4 UI 一致性加固 (mobile 跟 web 视觉语言对齐) | BUG-176 DeepSeek 思考泄漏止血 | 
| S83 | 2026-07-07 | **[已部署] v3.0.99 BUG-176 修 DeepSeek reasoning_content 泄漏到 analysis_report** — commit a2fe6ac. 修前 DeepSeek 返 reasoning_content (AI 内部思考) 被当正常 content 直接拼进 analysis_report 字段 → 用户看到 AI 思考内容污染报告. 修法: server 端 LLM provider 显式过滤 reasoning_content 字段只取 content. **跨项目通用铁律 #34** 新沉淀: DeepSeek 等 reasoning 模型的 reasoning_content 必过滤防泄漏到 analysis_report (跟 BUG-145 跨端 UI 防御 100% 同源: 输出层必做字段白名单, 不信任模型默认行为) | BUG-177 mobile 强制升级 modal 永远弹出 | 
| S84 | 2026-07-07 | **[已部署] v3.0.100 BUG-177 修 mobile 强制升级 modal 永远弹出** — commit 2148f9e. **根因**: apps/mobile/App.tsx 修前用 info.version (server-only 进程版本 e.g. v3.0.99) 跟 clientVer (mobile APK 版本 e.g. v3.0.98) 对比 → S83 server hotfix 后 server.version=v3.0.99 但公网 mobile APK 还是 v3.0.98 → 永远不等 → 强制升级 modal 永远弹 → APP 无法进主界面 (跟 server-only hotfix 设计矛盾: server hotfix 不需 rebuild APK, 客户端不应强制升级). **修法**: App.tsx line 297-306 client 跟 server 对比改用 info.mobileLatestApkVersion (server 从公网 APK 列表扫到的真实 APK 版本), 保留 info.version fallback 兜底. **跨项目通用铁律 2 条新沉淀**: (1) client 升级对比必用公网真实 APK version, 不用 server-only 进程 version; (2) server /api/version 必同时返 version + mobileLatestApkVersion 让 client 区分场景 (v3.0.62 BUG-131 已实现, 这次实战验证). 跨端 8 处版本号同步 3.0.99→3.0.100 (mobile version.ts + build.gradle versionCode 100→101/versionName + web version.ts APP_VERSION+APP_VERSION_CODE+APP_BUILD_DATE + server package.json + src/index.ts fallback + ecosystem.config.js env+env_production 2 处 + changelog.json + 远端 .env/systemd deploy.sh 自动同步) | BUG-178 分镜列表 UI 重设计 | 
| S85 | 2026-07-07 | **[已部署] v3.0.101/102 BUG-178 分镜列表 UI 重设计 + BUG-179 mobile loading hang** — commit 0c6551c (v3.0.101) + 174c321 (v3.0.102). **BUG-178**: 分镜列表 UI 重设计, 修移动端分镜内容无法修改. **跨项目通用铁律 #36** 新沉淀 (UI 一致性 + 可编辑性跨端对齐). **BUG-179**: web shots UI refactor (1 textarea TXT-like 风格) + mobile fix loading hang (修前 loading 状态挂起不退出). 跨端 8 处版本号同步 3.0.100→3.0.101→3.0.102 | BUG-180/181/182 web 乱码 + mobile 编辑 + commit 假报告补救 | 
| S86 | 2026-07-08 | **[已部署] v3.0.103/104 BUG-180 web 乱码 + BUG-181 mobile 编辑 + BUG-182 commit 假报告补救** — commit d49d3dc (v3.0.103) + 980b23f/a175e51/7dbba83 (v3.0.104 3 个 commit). **BUG-180**: web 登录页 title 末尾乱码 (GB2312 二级汉字如 "端" U+7AEF 渲染成 "端?"), 根因 Content-Type 头无 charset= 浏览器按 latin1 fallback. **BUG-181**: mobile 小说分析编辑 (3 editable cards 1:1 mirror web, ScriptDetailScreen.tsx). **BUG-182 (反讽实战, 跟 BUG-079 假报告 100% 同源)**: v3.0.103 commit d49d3dc 写了 BUG-181 但 ScriptDetailScreen.tsx last commit 是 v3.0.92 → commit message 假报告 (代码没真改). v3.0.104 a175e51 补 8 处版本号 + nginx charset 修复, 980b23f 补真正修改的 2 mobile 文件 (ScriptDetailScreen.tsx + 另一个), 7dbba83 补 a175e51 漏 commit 的 2 文件. **跨项目通用铁律 #37** 新沉淀: commit message 必跟代码 1:1 对应, 验收必跑 `git log -1 --format='%H %s' <被改文件路径>` 检查 drift. **#38** 新沉淀: nginx 必配 charset utf-8 (location / charset utf-8;) + 主进程 restart 而非 reload (mime.types 需主进程重新加载) + 3 兜底联动 (HTML meta charset=UTF-8 + nginx charset + Google Fonts Noto Sans SC). 跨端 8 处版本号同步 3.0.103→3.0.104 | 集数公式校准 + 角色分析重构 | 
| S87 | 2026-07-08 | **[已部署] v3.0.105/106 集数公式校准 + 角色分析系统重构 + BUG-183 部署记录** — commit ad0360e (v3.0.105) + e6ee195+63fe0e6 (v3.0.106). **v3.0.105 ad0360e**: 集数公式分母校准 3675→6667 (100 万字=150 集, 修前字数偏多集数偏少), 按剧情阶段权重分配原文 (高潮多给/铺垫少给), AI 规划 Prompt 六档梯度, 状态卡节奏标注 (规范修订). **v3.0.106 e6ee195**: 角色分析系统重构 — 5 类角色类型 × 4 种阵营 × 全角色补齐策略 (规范修订). **v3.0.106 63fe0e6**: 添加 BUG-183 部署问题记录 (tar 嵌套目录 + systemd ProtectSystem 导致部署路径错乱的踩坑沉淀). 跨端 8 处版本号同步 v3.0.104→3.0.105→3.0.106 | web LightboxImage / mobile tsc baseline 53 错清理 / 性能优化 (开放候选, 等 user 拍) |
