...
exports.videoAgentService = exports.VideoAgentService = void 0;
const agnesVideoProvider_1 = require("./agnesVideoProvider");
const agnesTextProvider_1 = require("./agnesTextProvider");
...
const imageAspectRatio_1 = require("../prompts/imageAspectRatio");
const videoAgentSystem_1 = require("../prompts/videoAgentSystem");
...
const videoBackgroundLocks = new Map();
// ── 计费 ──
...
const ALLOWED_DURATIONS = [5, 10, 15];
...
function partsToText(parts) {
    return parts.map(p => {
        ...
        if (p.type === 'plan')
            return `[方案: ${p.data.prompt}]`;
        if (p.type === 'question')
            return `[问题: ${p.data.question}]`;
        return '';
    }).filter(Boolean).join('\n');
}
function numFramesForDuration(sec, fps) {
    // 满足 8n+1
...
function pushStreamingProgress(messages, stage = 'generating') {
    ...
    for (let i = result.length - 1; i >= 0; i--) {
        ...
        if (hasPlan) {
            result[i] = { ...result[i], parts: [...result[i].parts, { type: 'streaming', stage }] };
            return result;
        }
    }
    return result;
...
function replaceStreamingPart(messages, newPart) {
    const result = [...messages];
    for (let i = result.length - 1; i >= 0; i--) {
        if (result[i].role !== 'assistant')
            continue;
        const hasStreaming = result[i].parts.some(p => p.type === 'streaming');
        if (hasStreaming) {
            result[i] = { ...result[i], parts: result[i].parts.map(p => p.type === 'streaming' ? newPart : p) };
            return result;
        }
    }
    for (let i = result.length - 1; i >= 0; i--) {
...
class VideoAgentService {
    async createConversation(userId) {
        const conversationId = await videoConversation_1.videoConversationModel.create({ userId, mode: 'text2vid' });
        const welcome = {
            id: (0, utils_1.generateUUID)(),
            role: 'assistant',
        ...
        await videoConversation_1.videoConversationModel.update(conversationId, {
            messages,
     ...
     *   - aspectRatio 独立参数
     */
    async processTurn(conversationId, userInputParts, aspectRatioFromClient, durationSecFromClient) {
        const conv = await videoConversation_1.videoConversationModel.findById(conversationId);
        if (!conv)
            throw new Error('会话不存在');
        if (conv.user_id === undefined)
            throw new Error('会话无 user_id');
        const messages = parseMessages(conv.messages);
        const userMessage = { id: (0, utils_1.generateUUID)(), role: 'user', parts: userInputParts, createdAt: Date.now() };
        messages.push(userMessage);
        // 1. 拿用户原文 (用户输入的 text part)
        const userText = partsToText(userInputParts);
        // 2. 拿参考图 URL
        const refImageUrls = (userInputParts || [])
            .filter(p => p.type === 'image' && p.role === 'reference')
            .map(p => p.url)
            .filter(Boolean);
        ...
        if (!ASPECT_DIMENSIONS[finalAspect]) {
            const textDims = (0, imageAspectRatio_1.parseAspectToDims)(finalAspect);
            if (textDims)
                finalAspect = `${textDims[0]}x${textDims[1]}`;
        }
        if (finalAspect !== aspectRatio) {
            ...
            logger_1.logger.info('VideoAgent: aspectRatio adjusted', {
        ...
        if (isModification && lastResultUrl && /^https?:\/\//.test(lastResultUrl)) {
            try {
                ...
                logger_1.logger.info('VideoAgent: processTurn synced cache + set local last_result_url', { conversationId, localUrl });
            }
            catch (err) {
                logger_1.logger.warn('VideoAgent: processTurn sync cache failed (fallback to agens URL)', {
        ...
        let promptOptimizedMode = null;
        let promptOptimizeUsage = null;
        if (finalPrompt && finalPrompt.length >= 3 && !isModification) {
            ...
            const isStoryboard = (0, videoAgentSystem_1.isStoryboardScript)(finalPrompt);
            const messages = isStoryboard
                ? (0, videoAgentSystem_1.buildStoryboardOptimizerMessages)(finalPrompt)
                : (0, videoAgentSystem_1.buildVideoPromptOptimizerMessages)(finalPrompt);
            promptOptimizedMode = isStoryboard ? 'storyboard' : 'generic';
            const t0 = Date.now();
            try {
                const llmPromise = agnesTextProvider_1.agnesTextProvider.chatCompletion({
                    messages,
                    temperature: isStoryboard ? 0.5 : 0.7, // 分镜需要更稳定, 降低 temperature
                    maxTokens: isStoryboard ? 1500 : 800, // 分镜允许更长输出
                    enableThinking: false,
                });
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('video-prompt-llm-timeout')), VIDEO_PROMPT_LLM_TIMEOUT_MS));
                ...
                if (llmOutput.length >= VIDEO_PROMPT_LLM_MIN_OUTPUT) {
                    ...
                    try {
                        await billingService_1.billingService.chargeImage(conv.user_id, VIDEO_PROMPT_LLM_COST, isStoryboard ? 'video prompt LLM 优化(分镜)' : 'video prompt LLM 优化', conversationId);
                    }
                    catch (chargeErr) {
                        logger_1.logger.warn('VideoAgent: prompt optimize charge failed (non-blocking)', {
                            conversationId,
                            error: chargeErr.message,
                        });
                    }
                    logger_1.logger.info('VideoAgent: prompt optimized by LLM', {
                        conversationId,
                    ...
                    logger_1.logger.warn('VideoAgent: LLM output too short, fallback to original', {
                        conversationId,
                        mode: promptOptimizedMode,
                ...
                logger_1.logger.warn('VideoAgent: LLM prompt optimization failed, fallback to original passthrough', {
                    conversationId,
                    mode: promptOptimizedMode,
        ...
        logger_1.logger.info('VideoAgent: processTurn', {
            conversationId,
    ...
    async confirm(conversationId) {
        ...
        if (videoBackgroundLocks.has(conversationId)) {
            logger_1.logger.warn('VideoAgent: confirm rejected, background lock held', { conversationId });
        ...
        videoBackgroundLocks.set(conversationId, new Promise(() => { }));
        try {
            const conv = await videoConversation_1.videoConversationModel.findById(conversationId);
            if (!conv)
                throw new Error('会话不存在');
            if (conv.status !== 'plan_ready')
                throw new Error(`状态 ${conv.status} 不可确认, 需 plan_ready`);
            const plan = parsePlan(conv.plan);
            if (!plan || !plan.prompt)
                throw new Error('会话无 plan');
            const durationSec = plan.durationSec || DEFAULT_DURATION_SEC;
            // v3.0.0.31 (S51): VIP + duration 矩阵计费 + 余额守门
            const user = await user_1.userModel.findById(conv.user_id);
            const isVip = (0, billingService_1.isVipActive)(user);
            const chargedAmount = (0, billingService_1.chargingForVideo)(isVip, durationSec);
            if (chargedAmount > 0) {
            ...
            logger_1.logger.info('VideoAgent: S51 charge preview', { userId: conv.user_id, isVip, durationSec, chargedAmount });
            const width = plan.width || 1152;
            const height = plan.height || 768;
            const fps = plan.fps || 24;
            const refUrls = plan.refImageUrls || [];
            // v3.0.0.15: i2v 模式 — modification 时用 last_result_url 作 i2v source
            const i2vSourceUrl = plan.i2vSourceUrl;
            ...
            setImmediate(() => {
                const bgPromise = this.runCreateTaskInBackground(conversationId, plan, useI2V ? i2vSourceUrl : (refUrls.length === 1 ? refUrls[0] : undefined), useI2V ? undefined : (refUrls.length > 1 ? refUrls : undefined), useI2V, originalPlan, durationSec, width, height, fps, refUrls)
                    .catch(err => {
                    logger_1.logger.error('VideoAgent: runCreateTaskInBackground failed', { conversationId, error: err.message });
            ...
            const taskId = await videoConversation_1.videoGenerationModel.create({
                conversationId,
                prompt: plan.prompt,
            ...
            logger_1.logger.info('VideoAgent: confirm accepted, createTask running in background', {
     ...
     */
    async runCreateTaskInBackground(conversationId, plan, image, images, useI2V, originalPlan, durationSec, width, height, fps, refUrls) {
        let createResult;
        try {
            // 调 agnes video 创建任务
            // v3.0.0.15: i2v 模式 — 用 last_result_url 作 image, 不混 user ref
            createResult = await agnesVideoProvider_1.agnesVideoProvider.createTask({
                prompt: plan.prompt,
                image,
                images,
                width, height,
            ...
            logger_1.logger.error('VideoAgent: agnes createTask failed (background), rolling back to plan_ready', {
            ...
            const curConv = await videoConversation_1.videoConversationModel.findById(conversationId);
            const failMessages = curConv
                ? replaceStreamingPart(parseMessages(curConv.messages), { type: 'error', message: friendlyMsg })
                : [];
            await videoConversation_1.videoConversationModel.update(conversationId, {
                status: 'plan_ready',
                plan: originalPlan,
        ...
        try {
            ...
            await videoConversation_1.videoConversationModel.update(conversationId, {
                ...
                resolution: `${width}x${height}`,
                fps,
                plan: { ...plan, _agnesVideoId: createResult.videoId },
            });
        }
        catch (err) {
            logger_1.logger.error('VideoAgent: persist taskId/videoId failed (background)', {
                conversationId, error: err.message,
            });
            await videoConversation_1.videoConversationModel.update(conversationId, {
                status: 'plan_ready',
        ...
        logger_1.logger.info('VideoAgent: createTask done (background), polling started', {
            conversationId, videoId: createResult.videoId.slice(0, 60),
        });
    }
    /** 后台轮询 — 起步 5s, 失败 backoff 到 30s 上限, 连续失败 5 次暂停 */
    startPolling(conversationId, videoId) {
        let attempts = 0;
        ...
        const tick = async () => {
            attempts++;
            try {
                const status = await agnesVideoProvider_1.agnesVideoProvider.queryStatus(videoId);
                consecutiveFailures = 0;
                currentInterval = POLLING_INTERVAL_MS; // 成功一次回到 5s
                const conv = await videoConversation_1.videoConversationModel.findById(conversationId);
                if (!conv)
                    return; // timer already cleared
                // v3.0.0.31 (S51): VIP + duration 矩阵计费 + 成功生成后真扣费
                const durationSecVal = conv.duration_sec || DEFAULT_DURATION_SEC;
                const user = await user_1.userModel.findById(conv.user_id);
                const isVip = (0, billingService_1.isVipActive)(user);
                const chargedAmount = (0, billingService_1.chargingForVideo)(isVip, durationSecVal);
                if (status.status === 'completed' && status.videoUrl) {
                    ...
                    if (chargedAmount > 0) {
                        ...
                        const chargeResult = await billingService_1.billingService.recordConsumption(conv.user_id, {
                            refType: 'video',
                        ...
                        if (!chargeResult) {
                            ...
                            logger_1.logger.error('VideoAgent: S69 chargeVideo failed (balance insufficient, video already generated)', {
                 ...
                 VALUES (?, ?, 'consumption_pending', ?, ?, ?, ?, 0, ?)`, [(0, utils_1.generateUUID)(), conv.user_id, chargedAmount, user?.balance || 0, conversationId, `视频生成(${durationSecVal}s${isVip ? '/VIP' : '/普通'}) - 待结算`, Date.now()]);
                            websocket_1.websocketService.broadcastBalanceUpdate(conversationId, user?.balance || 0);
                        }
                        else {
                            logger_1.logger.info('VideoAgent: S51 charged', {
                        ...
                        logger_1.logger.info('VideoAgent: S51 free (5s or VIP 10s)', { conversationId, durationSecVal, isVip });
                        await videoConversation_1.videoConversationModel.update(conversationId, {
                            billing_status: 'settled',
                        });
                    }
                    ...
                    try {
                        ...
                        const genRow = await queryOne(`SELECT id FROM video_generations WHERE conversation_id = ? ORDER BY created_at DESC LIMIT 1`, [conversationId]);
                        if (genRow?.id) {
                            await videoConversation_1.videoGenerationModel.update(genRow.id, {
                                charged_amount: chargedAmount,
                            });
                        }
                    }
                    catch (err) {
                        logger_1.logger.warn('VideoAgent: video_generations.charged_amount update failed', { error: err.message });
                    }
                    logger_1.logger.info('VideoAgent: polling completed', { conversationId, videoId, videoUrl: status.videoUrl.slice(0, 60) });
                    ...
                    try {
                        ...
                        logger_1.logger.info('VideoAgent: cache done, last_result_url → local', { conversationId, localUrl });
                    }
                    catch (err) {
                        logger_1.logger.warn('VideoAgent: cacheVideoToLocal failed (non-fatal, keep agens URL)', {
                            conversationId, error: err.message,
                        });
                    }
                    return; // stop
                    ...
                    const newStatus = retryCount < 3 ? 'plan_ready' : 'tool_failed';
                    ...
                    logger_1.logger.warn('VideoAgent: polling failed', { conversationId, videoId, retryCount, error: status.error });
                    return; // stop
                }
                else {
                    // in_progress / queued
                    ...
                    logger_1.logger.debug('VideoAgent: polling progress', { conversationId, videoId, progress: status.progress, interval: currentInterval });
                }
            }
            catch (err) {
                consecutiveFailures++;
                ...
                logger_1.logger.warn('VideoAgent: polling error, backing off', {
                ...
                if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                    ...
                    logger_1.logger.error('VideoAgent: polling throttled, stopped', { conversationId, videoId, consecutiveFailures });
                    return; // stop
                }
            ...
            if (attempts >= MAX_POLLING_ATTEMPTS) {
                ...
                logger_1.logger.error('VideoAgent: polling timeout', { conversationId, videoId });
                return; // stop
            }
    ...
    async cacheVideoToLocal(conversationId, videoUrl) {
        try {
            ...
            if (fs.existsSync(filePath) && fs.statSync(filePath).size > 0) {
                logger_1.logger.info('VideoAgent: cacheVideoToLocal already exists, skip', { conversationId, filePath });
                return;
            }
            logger_1.logger.info('VideoAgent: cacheVideoToLocal start', { conversationId, videoUrl, filePath });
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 5 * 60 * 1000); // 5 min timeout
            const upstream = await fetch(videoUrl, { signal: controller.signal });
            clearTimeout(timeout);
            if (!upstream.ok || !upstream.body) {
                logger_1.logger.warn('VideoAgent: cacheVideoToLocal upstream not ok', { conversationId, status: upstream.status });
                return;
            }
            ...
            logger_1.logger.info('VideoAgent: cacheVideoToLocal done', {
                conversationId, filePath, totalBytes, relPath,
            });
        }
        catch (err) {
            logger_1.logger.error('VideoAgent: cacheVideoToLocal error', {
...
exports.VideoAgentService = VideoAgentService;
exports.videoAgentService = new VideoAgentService();
