...
exports.agnesVideoProvider = exports.AgnesVideoProvider = void 0;
const logger_1 = require("../utils/logger");
const ffmpegHelper_1 = require("../utils/ffmpegHelper");
...
class AgnesVideoProvider {
    ...
    constructor(apiKey) {
        ...
        if (!this.apiKey)
            logger_1.logger.warn('AGNES_API_KEY (或 AGNES_IMAGE_API_KEY) not set for video');
    }
    ...
    async inlineIfLocal(url) {
        ...
        if (httpMatch) {
            try {
                ...
                if (u.pathname.startsWith('/api/agent/uploads/') || u.pathname.startsWith('/api/agent/video-local/')) {
                    ...
                    logger_1.logger.info('AgnesVideoProvider: normalized shipin-APP URL to relative', { original: url, normalized });
                }
                else {
        ...
        if (imgMatch) {
            ...
            if (!fs.existsSync(filePath)) {
                logger_1.logger.warn('AgnesVideoProvider: ref file not found on disk', { filePath });
                return url;
            }
            const buf = fs.readFileSync(filePath);
            // v3.0.0.18: agnes video 期待纯 base64 字符串 (不带 data: 前缀)
            const base64 = buf.toString('base64');
            logger_1.logger.info('AgnesVideoProvider: inlined ref image to base64', { filename: imgMatch[2], bytes: buf.length, b64Len: base64.length });
            return base64;
        }
        ...
        if (vidMatch) {
            ...
            if (!fs.existsSync(filePath)) {
                logger_1.logger.warn('AgnesVideoProvider: video local not found on disk', { filePath });
                return url;
            }
            try {
                const result = (0, ffmpegHelper_1.extractFirstFrameAsPngBase64)(filePath);
                logger_1.logger.info('AgnesVideoProvider: extracted first frame for i2v', {
                    filename: vidMatch[2],
                ...
                logger_1.logger.error('AgnesVideoProvider: ffmpeg frame extraction failed', { filePath, error: err.message });
    ...
    async createTask(opts) {
        ...
        if (opts.image) {
            ...
            logger_1.logger.info('AgnesVideoProvider: createTask image (inlined)', {
        ...
        logger_1.logger.info('AgnesVideoProvider: createTask', {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                if (response.status === 429 || response.status === 503 || (response.status >= 500 && response.status < 600)) {
                    ...
                    logger_1.logger.warn('AgnesVideoProvider: createTask retryable error', {
                ...
                if (!videoId)
                    throw new Error('Agnes video response missing video_id');
                logger_1.logger.info('AgnesVideoProvider: createTask done', {
                ...
                if (attempt < MAX_RETRIES - 1) {
                    logger_1.logger.warn('AgnesVideoProvider: createTask network error, will retry', {
                        attempt, error: lastError.message.slice(0, 100),
                    });
                    await new Promise(r => setTimeout(r, RETRY_BACKOFF_MS[attempt]));
                    continue;
                }
...
exports.AgnesVideoProvider = AgnesVideoProvider;
exports.agnesVideoProvider = new AgnesVideoProvider();
