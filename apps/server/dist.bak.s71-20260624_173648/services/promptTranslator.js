...
exports.translateCharacterDescriptionToEnglish = translateCharacterDescriptionToEnglish;
 ...
 */
async function translateCharacterDescriptionToEnglish(zhText) {
    ...
    if (asciiCount / trimmed.length > 0.8) {
        logger_1.logger.info('promptTranslator: skip, already english', { len: trimmed.length });
        return trimmed;
    }
    try {
        const result = await agnesTextProvider_1.agnesTextProvider.chatCompletion({
            messages: [
        ...
        logger_1.logger.info('promptTranslator: translated', {
        ...
        logger_1.logger.warn('promptTranslator: failed, using fallback (original + photographic)', {
            originalLen: trimmed.length,
