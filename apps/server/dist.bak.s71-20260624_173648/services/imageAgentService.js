...
exports.imageAgentService = exports.ImageAgentService = void 0;
...
const imagePromptBuilder_1 = require("../prompts/imagePromptBuilder");
...
const DEFAULT_IMG_DIM = [1024, 1024];
...
const imageBackgroundLocks = new Map();
...
function estimateCharging(mode, hasRef) {
    return 0;
}
function refImageUrlsFromParts(parts) {
    return parts
...
class ImageAgentService {
    ...
    async createConversation(userId) {
        const conversationId = await imageConversation_1.imageConversationModel.create({ userId, mode: 'text2img' });
        const welcome = {
            id: (0, utils_1.generateUUID)(),
            role: 'assistant',
            parts: [{
        ...
        await imageConversation_1.imageConversationModel.update(conversationId, {
            messages,
            plan: null,
            planFields: {},
     ...
     */
    async processTurn(conversationId, userInputParts, aspectRatioFromClient) {
        const conv = await imageConversation_1.imageConversationModel.findById(conversationId);
        if (!conv)
        ...
        const refUrlsAccum = refImageUrlsFromParts(messages.filter(m => m.role === 'user').flatMap(m => m.parts));
        ...
        if (userTextRaw && userTextRaw.length >= 3 && !isModification) {
            const isStoryboard = (0, videoAgentSystem_1.isStoryboardScript)(userTextRaw);
            const messages = isStoryboard
                ? (0, videoAgentSystem_1.buildStoryboardOptimizerMessages)(userTextRaw)
                : (0, videoAgentSystem_1.buildVideoPromptOptimizerMessages)(userTextRaw);
            ...
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('image-prompt-llm-timeout')), IMG_PROMPT_LLM_TIMEOUT_MS));
                ...
                if (llmOutput.length >= IMG_PROMPT_LLM_MIN_OUTPUT) {
                    ...
                    try {
                        await billingService_1.billingService.chargeImage(conv.user_id, IMG_PROMPT_LLM_COST, isStoryboard ? 'image prompt LLM 优化(分镜)' : 'image prompt LLM 优化', conversationId, 'prompt_optimize', isStoryboard ? '图片 prompt LLM 优化(分镜)' : '图片 prompt LLM 优化');
                    }
                    catch (chargeErr) {
                        logger_1.logger.warn('ImageAgent: prompt optimize charge failed (non-blocking)', {
                            conversationId, error: chargeErr.message,
                        });
                    }
                    logger_1.logger.info('ImageAgent: prompt optimized by LLM', {
                        conversationId,
                    ...
                    logger_1.logger.warn('ImageAgent: LLM output too short, fallback to original', {
                        conversationId, mode: promptOptimizedMode, outputLen: llmOutput.length,
                    });
                }
            }
            catch (llmErr) {
                logger_1.logger.warn('ImageAgent: LLM prompt optimization failed, fallback to passthrough', {
                    conversationId, mode: promptOptimizedMode, error: llmErr.message,
                });
            }
        }
        // 移除比例后缀 (兼容老调用) — 如果包含 "比例换成" 之类的关键词, parse 出来
        const aspectRatioParsed = (0, imageAspectRatio_1.parseAspectRatioFromText)(userTextRaw);
        let finalAspectRatio = aspectRatioFromClient || aspectRatioParsed || conv.plan?.aspectRatio || '1024x1024';
        // v3.0.0.18 (audit #12): 文字比例 (9:16/16:9 等) 兜底成 WxH, 跟 video 保持一致
        const dimsFromText = (0, imageAspectRatio_1.parseAspectToDims)(finalAspectRatio);
        if (dimsFromText)
            finalAspectRatio = `${dimsFromText[0]}x${dimsFromText[1]}`;
        ...
        const aiParts = [
            { type: 'plan', data: { prompt: enPromptBase, aspectRatio: finalAspectRatio, style: '', refImageUrls: refUrlsAccum, planFields: { subject: enPromptBase, negative: existingFields.negative || '' } } },
        ...
        await imageConversation_1.imageConversationModel.update(conversationId, {
            messages,
            plan: { prompt: enPromptBase, aspectRatio: finalAspectRatio, refImageUrls: refUrlsAccum, style: '' },
            planFields: newPlanFields,
            status: 'plan_ready', // v3.0.0.16: PR-M 极简模式直接 plan_ready (不再需要 translatePlan 翻译步骤)
        });
        logger_1.logger.info('ImageAgent: processTurn', {
            conversationId,
            isModification,
     ...
     */
    async translatePlan(conversationId) {
        const conv = await imageConversation_1.imageConversationModel.findById(conversationId);
        if (!conv)
            throw new Error(`会话不存在: ${conversationId}`);
        ...
        const { enPrompt, negative } = (0, imagePromptBuilder_1.buildFinalEnglishPrompt)(userText, userNegative);
        // 3. 状态: plan_translating → plan_ready
        const planObj = {
            prompt: enPrompt,
            aspectRatio: conv.plan?.aspectRatio || '1024x1024',
            style: '',
            refImageUrls: conv.plan?.refImageUrls || [],
            negative,
        ...
        logger_1.logger.info('ImageAgent: translatePlan done', {
            conversationId,
     ...
     */
    async updatePlanFields(conversationId, updates) {
        const conv = await imageConversation_1.imageConversationModel.findById(conversationId);
        if (!conv)
            throw new Error(`会话不存在: ${conversationId}`);
        ...
        return { conversationId, status: 'plan_cn_ready', planFields: updated };
    }
    ...
    async confirm(conversationId) {
        ...
        if (imageBackgroundLocks.has(conversationId)) {
            logger_1.logger.warn('ImageAgent: confirm rejected, background lock held', { conversationId });
            throw new Error('上一个生图任务还在进行中, 请稍候再试');
        }
        // 占位 promise, 让 has() 立即命中
        imageBackgroundLocks.set(conversationId, new Promise(() => { }));
        try {
            const conv = await imageConversation_1.imageConversationModel.findById(conversationId);
            if (!conv)
                throw new Error(`会话不存在: ${conversationId}`);
            if (conv.status !== 'plan_ready')
                throw new Error(`当前状态 ${conv.status} 不可确认, 需 plan_ready`);
            const plan = parsePlan(conv.plan);
            if (!plan || !plan.prompt)
                throw new Error('会话无 plan, 无法生成');
            ...
            const dailyCount = await billingService_1.billingService.imageDailyCount(conv.user_id);
            const quota = isVip ? billingService_1.IMAGE_DAILY_QUOTA_VIP : billingService_1.IMAGE_DAILY_QUOTA_STANDARD;
            if (dailyCount >= quota) {
                logger_1.logger.warn('ImageAgent: daily quota exceeded', { userId: conv.user_id, dailyCount, quota, isVip });
                throw new Error(isVip
                    ? `今日生图已达 ${dailyCount} 张 (VIP 限额异常, 请联系客服)`
                    : `今日生图已达 ${dailyCount} 张上限, 升级 VIP 解锁无限生成`);
            }
            logger_1.logger.info('ImageAgent: free (S51 quota check passed)', {
            ...
            await imageConversation_1.imageConversationModel.update(conversationId, {
                status: 'tool_queued',
                messages: preConfirmMsgs,
            });
            // 3. 写 image_generations 审计 (queued)
            const taskId = await imageConversation_1.imageGenerationModel.create({
                conversationId,
                prompt: plan.prompt,
            ...
            const bgPromise = this.runImageGenerationBackground(conversationId, taskId, plan, conv.last_result_url || null, amount)
                .catch((err) => {
                logger_1.logger.error('ImageAgent: background run failed (unexpected)', {
                    conversationId, taskId, error: err.message,
                });
            })
                .finally(() => {
            ...
            logger_1.logger.info('ImageAgent: confirm queued, background started', {
     ...
     */
    async runImageGenerationBackground(conversationId, taskId, plan, lastResultUrl, amount) {
        try {
            // 1) 收集 ref: 优先 user ref, 没有则 last_result_url
            const userRefUrls = (plan.refImageUrls || []).slice(0, 1);
            const isModification = !!lastResultUrl;
            const refImages = userRefUrls.length > 0
                ? userRefUrls
            ...
            const dims = (0, imageAspectRatio_1.parseAspectToDims)(plan.aspectRatio) || DEFAULT_IMG_DIM;
            const [w, h] = dims;
            logger_1.logger.info('ImageAgent: background run start', {
            ...
            const prevMessages = parseMessages((await imageConversation_1.imageConversationModel.findById(conversationId))?.messages);
            const messages = replaceStreamingPart(prevMessages, {
                type: 'image', url: result.url, role: 'result',
            });
            await imageConversation_1.imageConversationModel.update(conversationId, {
            ...
            await imageConversation_1.imageGenerationModel.update(taskId, {
                status: 'completed',
            ...
            const convForLog = await imageConversation_1.imageConversationModel.findById(conversationId);
            if (convForLog) {
                await billingService_1.billingService.recordConsumption(convForLog.user_id, {
                    refType: 'image',
                    refId: taskId,
                    ...
                    logger_1.logger.warn('ImageAgent: recordConsumption failed (non-blocking)', {
                        conversationId, taskId, error: e?.message,
                    });
                });
            }
            logger_1.logger.info('ImageAgent: background run done', {
                conversationId, taskId, resultUrl: result.url.slice(0, 80),
            });
        }
        catch (err) {
            const errMsg = err?.message || String(err);
            logger_1.logger.error('ImageAgent: background run failed', {
...
exports.ImageAgentService = ImageAgentService;
exports.imageAgentService = new ImageAgentService();
