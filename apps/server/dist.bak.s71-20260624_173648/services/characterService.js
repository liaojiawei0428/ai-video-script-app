...
exports.normalizeOldDescriptionFormat = normalizeOldDescriptionFormat;
 ...
 */
function mapRoleTypeToLegacy(cnRoleType) {
    if (!cnRoleType)
        return 'supporting';
    ...
    if (t === 'protagonist' || t === 'antagonist' || t === 'supporting' || t === 'minor')
        return t;
    return 'supporting';
}
/**
 ...
 */
function extractEraContext(analysisReport) {
    if (!analysisReport)
        return '';
...
function lockGeneration(characterId) {
    if (inProgressGenerations.has(characterId))
        return false;
    const timer = setTimeout(() => {
        inProgressGenerations.delete(characterId);
        logger_1.logger.warn('generateImageVariants: lock auto-expired', { characterId });
    }, 5 * 60 * 1000);
    inProgressGenerations.set(characterId, timer);
    return true;
}
function unlockGeneration(characterId) {
    const timer = inProgressGenerations.get(characterId);
    if (timer)
        clearTimeout(timer);
    inProgressGenerations.delete(characterId);
}
function isGenerationLocked(characterId) {
...
async function extractDescriptions(novelId, fullSummary, novelTitle, styleId = 'realistic') {
    ...
    if (!fullSummary) {
        logger_1.logger.warn('extractDescriptions: 全文摘要和分析报告均为空, 跳过', { novelId });
        return { total: 0, succeeded: 0, failed: 0, characters: [] };
    }
    // 推送状态
    ...
    websocket_1.websocketService.broadcastChunkProgress(novelId, {
    ...
    const userPrompt = buildCharacterDescriptionUserPrompt(fullSummary, characters.map(c => c.name), novelTitle || '未命名', novel?.novelExcerpts || undefined);
    ...
    try {
        ...
        parsedDescriptions = parsedDescriptions.map((d) => {
    ...
    for (const char of characters) {
        ...
        if (matched) {
            try {
                ...
                const newRoleType = mapRoleTypeToLegacy(matched.roleType || char.roleType);
                await (0, db_1.execute)(`UPDATE characters SET description = ?, extra_description = ?, role_type = ?, style_id = ?, confirmed = 0, image_gen_status = 'none' WHERE id = ?`, [
                    description,
                    extraDescription,
                    newRoleType,
                    styleId,
                ...
                result.push({
                    ...
                    description: { name: char.name, description, extraDescription },
                    extraDescription: { name: char.name, description: '', extraDescription },
                });
                succeeded++;
            }
            catch (err) {
            ...
            try {
                ...
                await (0, db_1.execute)(`UPDATE characters SET description = ?, extra_description = ?, role_type = ?, style_id = ?, confirmed = 0, image_gen_status = 'none' WHERE id = ?`, [fallbackDesc, fallbackExtra, fallbackRoleType, styleId, char.id]);
                result.push({
                    id: char.id,
                    name: char.name,
                    roleType: fallbackRoleType,
                    description: { name: char.name, description: fallbackDesc, extraDescription: '' },
                    extraDescription: { name: char.name, description: '', extraDescription: '' },
                });
                succeeded++;
            }
    ...
    if (confirmedIds.length > 0) {
        await (0, db_1.execute)(`UPDATE characters SET confirmed = 1, confirmed_at = ? WHERE id IN (${confirmedIds.map(() => '?').join(',')})`, [Date.now(), ...confirmedIds]);
        logger_1.logger.info('extractDescriptions: auto-confirmed', { novelId, count: confirmedIds.length });
    }
    // 推送状态
 ...
 * 输出: markdown 文本 "# 基本信息\n- name: X\n- age: 18\n..." 格式
 */
function normalizeOldDescriptionFormat(s) {
 ...
 */
function extractDistinctiveFeatures(text) {
    if (!text)
        return '';
    // 找包含关键词的段落 (## 标题 或 - 列表项)
    const patterns = [
...
async function confirmDescription(characterId, userEdits) {
    ...
    await (0, db_1.execute)(`UPDATE characters SET confirmed = 1, confirmed_at = ? WHERE id = ?`, [Date.now(), characterId]);
...
async function generateImageVariants(characterId, userId, options = {}) {
    ...
    try {
        ...
        const { buildCharacterSheetPrompt } = await Promise.resolve().then(() => __importStar(require('./characterSheetPrompt')));
        const { buildStyleAnchorPrefix, buildStyleNegativePrompt, buildStyleBibleJsonBlock } = await Promise.resolve().then(() => __importStar(require('./styleBible')));
        ...
        const { translateCharacterDescriptionToEnglish } = await Promise.resolve().then(() => __importStar(require('./promptTranslator')));
        const translatedVisualText = await translateCharacterDescriptionToEnglish(visualText);
        // v2.5.9: 获取 styleBible 并注入
        const novelWithBible = await (0, db_1.queryOne)('SELECT style_bible FROM novels WHERE id = ?', [char.novelId]);
        ...
        logger_1.logger.info('character sheet prompt translated', {
            characterId,
        ...
        const sheetPrompt = buildCharacterSheetPrompt(sheetData, styleBible);
        const finalPrompt = sheetPrompt;
        logger_1.logger.info('generateImageVariants: character sheet prompt', {
        ...
        const totalCost = billingService_1.CHARACTER_VARIANT_PRICE;
        ...
        const quota = await billingService_1.billingService.checkImageQuota(userId);
        if (!quota.allowed) {
            await (0, db_1.execute)(`UPDATE characters SET image_gen_status = 'failed' WHERE id = ?`, [characterId]);
            throw new Error(`生图日配额已用完 (已生成 ${quota.used}/${quota.quota} 张), 开通 VIP 享无限生图`);
        }
        // ========== 单次调用生成三视图角色卡 ==========
        const { getDefaultImageProvider } = await Promise.resolve().then(() => __importStar(require('./imageProvider')));
        ...
        try {
            ...
            totalSucceeded = 1;
            totalFailed = 0;
            logger_1.logger.info('generateImageVariants: sheet done', {
                characterId, name: char.name, durationMs: result.durationMs,
                urlPrefix: result.url.slice(0, 80),
            });
        }
        catch (err) {
            logger_1.logger.error('generateImageVariants: sheet failed', {
        ...
        if (totalSucceeded > 0) {
            const chargedAmount = totalSucceeded * billingService_1.CHARACTER_VARIANT_PRICE;
            // v3.0.32 BUG-078 S71: 加 refType + refLabel, 走统一 recordConsumption
            const chargeResult = await billingService_1.billingService.chargeImage(userId, chargedAmount, `角色图片生成(${totalSucceeded}张) - ${char.name}`, char.novelId, 'character_variant', `角色三视图《${char.name}》(${totalSucceeded}张)`);
            if (!chargeResult) {
                logger_1.logger.error('characterService.generateImageVariants: chargeImage failed (should not happen, balance was prechecked)', {
                    characterId, userId, totalSucceeded, chargedAmount,
                });
            }
            else {
                logger_1.logger.info('characterService.generateImageVariants: S69 charged', {
        ...
        // 写库（三视图只要 1 张就算完成）
        const finalStatus = totalSucceeded >= 1 ? 'completed' : 'failed';
        ...
        logger_1.logger.info('generateImageVariants: 完成', {
            ...
            succeeded: totalSucceeded,
            failed: totalFailed,
            charged: totalSucceeded * billingService_1.CHARACTER_VARIANT_PRICE,
        });
        return {
            characterId,
            characterName: char.name,
...
async function generateImageForShot(shotId, userId) {
    ...
    const charDesc = characters.map((c) => {
        ...
        if (desc.prompt_safe_description)
            return desc.prompt_safe_description;
        // fallback: 用详细字段拼接
    ...
    // 注入风格圣经
    const { buildStyleAnchorPrefix: buildAnchor } = await Promise.resolve().then(() => __importStar(require('./styleBible')));
    ...
    if (!user)
        throw new Error('用户不存在');
    if ((user.balance || 0) < billingService_1.CHARACTER_VARIANT_PRICE) {
        throw new Error(`余额不足, 需要 ¥${billingService_1.CHARACTER_VARIANT_PRICE.toFixed(2)}`);
    }
    ...
    const chargeResult = await billingService_1.billingService.chargeImage(userId, billingService_1.CHARACTER_VARIANT_PRICE, `镜头图片生成 - ${shotId}`, novel?.id || '');
    const balanceAfter = chargeResult?.balanceAfter ?? Math.round(((user.balance || 0) - billingService_1.CHARACTER_VARIANT_PRICE) * 100) / 100;
    // 写库
    ...
    logger_1.logger.info('generateImageForShot: 完成', { shotId, charged: billingService_1.CHARACTER_VARIANT_PRICE });
    return { shotId, imageUrl: result.url, charged: billingService_1.CHARACTER_VARIANT_PRICE };
}
 ...
 */
function relabelEnglishKeys(text) {
    if (!text)
        return text;
    const KEY_LABEL = {
...
function mapRowToCharacterV2(row) {
    ...
    if (row.description) {
        base.description = typeof row.description === 'string' ? row.description : JSON.stringify(row.description);
    }
    if (row.extra_description) {
        base.extraDescription = typeof row.extra_description === 'string' ? row.extra_description : JSON.stringify(row.extra_description);
    }
    if (row.style_id)
