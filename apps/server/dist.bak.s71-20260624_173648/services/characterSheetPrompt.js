...
exports.buildCharacterSheetPrompt = buildCharacterSheetPrompt;
exports.buildSingleAnglePrompt = buildSingleAnglePrompt;
function buildEnglishVisualDescription(data) {
    const parts = [];
    if (data.gender)
        parts.push(`a ${data.gender === '男' ? 'male' : 'female'} character`);
    if (data.age)
        parts.push(`aged ${data.age}`);
    if (data.height)
        parts.push(`${data.height} tall`);
    if (data.build)
        parts.push(`with ${data.build} body type`);
    const faceParts = [];
    if (data.face)
        faceParts.push(`${data.face} face shape`);
    if (data.eyes)
        faceParts.push(`${data.eyes} eyes`);
    if (data.eyebrows)
        faceParts.push(`${data.eyebrows} eyebrows`);
    if (data.nose)
        faceParts.push(`${data.nose} nose`);
    if (data.lips)
        faceParts.push(`${data.lips} lips`);
    if (data.skin)
        faceParts.push(`${data.skin} skin`);
    if (faceParts.length > 0)
        parts.push(faceParts.join(', '));
    const hairParts = [];
    if (data.hair_color)
        hairParts.push(`${data.hair_color} hair color`);
    if (data.hair_style)
        hairParts.push(`${data.hair_style} hairstyle`);
    if (data.hair_length)
        hairParts.push(`${data.hair_length} length`);
    if (data.hair_texture)
        hairParts.push(`${data.hair_texture} hair texture`);
    if (data.hair_accessories)
        hairParts.push(`wearing ${data.hair_accessories}`);
    if (hairParts.length > 0)
        parts.push(hairParts.join(', '));
    const clothingParts = [];
    if (data.clothing_top)
        clothingParts.push(`top: ${data.clothing_top}`);
    if (data.clothing_bottom)
        clothingParts.push(`bottom: ${data.clothing_bottom}`);
    if (data.clothing_outer)
        clothingParts.push(`outerwear: ${data.clothing_outer}`);
    if (data.clothing_shoes)
        clothingParts.push(`shoes: ${data.clothing_shoes}`);
    if (clothingParts.length > 0)
        parts.push(`wearing ${clothingParts.join('; ')}`);
    const accessoryParts = [];
    if (data.accessories_neck)
        accessoryParts.push(`neck: ${data.accessories_neck}`);
    if (data.accessories_ears)
        accessoryParts.push(`ears: ${data.accessories_ears}`);
    if (data.accessories_hands)
        accessoryParts.push(`hands: ${data.accessories_hands}`);
    if (data.accessories_waist)
        accessoryParts.push(`waist: ${data.accessories_waist}`);
    if (data.accessories_other)
        accessoryParts.push(`other: ${data.accessories_other}`);
    if (accessoryParts.length > 0)
        parts.push(`accessories - ${accessoryParts.join('; ')}`);
    if (data.props)
        parts.push(`holding or near ${data.props}`);
    if (data.distinctive_features)
        parts.push(`distinctive features: ${data.distinctive_features}`);
    if (data.makeup)
        parts.push(`makeup: ${data.makeup}`);
    if (data.default_expression)
        parts.push(`expression: ${data.default_expression}`);
    return parts.join(', ');
}
function buildChineseVisualDescription(data) {
    const parts = [];
    const headParts = [];
    if (data.face)
        headParts.push(`${data.face}脸型`);
    if (data.eyes)
        headParts.push(`${data.eyes}眼睛`);
    if (data.eyebrows)
        headParts.push(`${data.eyebrows}眉毛`);
    if (data.nose)
        headParts.push(`${data.nose}鼻子`);
    if (data.lips)
        headParts.push(`${data.lips}嘴唇`);
    if (data.skin)
        headParts.push(`${data.skin}肤色`);
    if (headParts.length > 0)
        parts.push(`面部：${headParts.join('，')}`);
    const hairParts = [];
    if (data.hair_color)
        hairParts.push(`${data.hair_color}发色`);
    if (data.hair_style)
        hairParts.push(`${data.hair_style}`);
    if (data.hair_length)
        hairParts.push(`${data.hair_length}长度`);
    if (data.hair_texture)
        hairParts.push(`${data.hair_texture}发质`);
    if (data.hair_accessories)
        hairParts.push(`佩戴${data.hair_accessories}`);
    if (hairParts.length > 0)
        parts.push(`发型：${hairParts.join('，')}`);
    const clothingParts = [];
    if (data.clothing_top)
        clothingParts.push(`上衣：${data.clothing_top}`);
    if (data.clothing_bottom)
        clothingParts.push(`下装：${data.clothing_bottom}`);
    if (data.clothing_outer)
        clothingParts.push(`外套：${data.clothing_outer}`);
    if (data.clothing_shoes)
        clothingParts.push(`鞋子：${data.clothing_shoes}`);
    if (clothingParts.length > 0)
        parts.push(`服装：${clothingParts.join('；')}`);
    const accessoryParts = [];
    if (data.accessories_neck)
        accessoryParts.push(`颈部：${data.accessories_neck}`);
    if (data.accessories_ears)
        accessoryParts.push(`耳部：${data.accessories_ears}`);
    if (data.accessories_hands)
        accessoryParts.push(`手部：${data.accessories_hands}`);
    if (data.accessories_waist)
        accessoryParts.push(`腰部：${data.accessories_waist}`);
    if (data.accessories_other)
        accessoryParts.push(`其他：${data.accessories_other}`);
    if (accessoryParts.length > 0)
        parts.push(`配饰：${accessoryParts.join('；')}`);
    if (data.props)
        parts.push(`随身道具：${data.props}`);
    if (data.distinctive_features)
        parts.push(`标志性特征：${data.distinctive_features}`);
    if (data.makeup)
        parts.push(`妆容：${data.makeup}`);
    if (data.default_expression)
        parts.push(`默认表情：${data.default_expression}`);
    return parts.join('。\n');
}
 ...
 */
function buildDynamicNegativeKeywords(styleId) {
    const base = [
 ...
 */
function buildPrimaryVisualBlock(data) {
...
function buildCharacterSheetPrompt(data, styleBible) {
    const styleId = data.styleId || 'realistic';
    ...
    const identityParts = [];
    if (data.name)
        identityParts.push(`character name: ${data.name}`);
    if (data.gender)
        identityParts.push(`gender: ${data.gender === '男' ? 'male' : 'female'}`);
    if (data.age)
        identityParts.push(`age: ${data.age}`);
    if (data.height)
        identityParts.push(`height: ${data.height}`);
    if (data.build)
        identityParts.push(`build: ${data.build}`);
    if (data.makeup)
        identityParts.push(`makeup style: ${data.makeup}`);
    const doNotChangeParts = [];
    if (data.do_not_change)
        doNotChangeParts.push(data.do_not_change);
    if (data.distinctive_features)
        doNotChangeParts.push(data.distinctive_features);
...
function buildSingleAnglePrompt(data, angle, styleBible) {
    const genreEn = styleBible?.visual.genre_en || 'ultra realistic, photorealistic, cinematic photography';
    const qualityEn = styleBible?.visual.quality_en || '8k uhd, high detail, sharp focus';
    const lightingEn = styleBible?.visual.lighting_en || 'studio soft light, three-point lighting';
    const colorEn = styleBible?.visual.colorStyle_en || 'natural color grading';
