...
exports.agnesTextProvider = exports.AgnesTextProvider = void 0;
...
class AgnesTextProvider {
    ...
    async chatCompletion(opts) {
        ...
        logger_1.logger.info('AgnesTextProvider: chatCompletion', {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                if (response.status === 429) {
                    ...
                    logger_1.logger.warn('AgnesTextProvider: rate limited', { attempt: attempt + 1, waitMs });
                ...
                if (content === null || content === undefined || content === '') {
                    if (reasoning && reasoning.trim().length > 0) {
                        logger_1.logger.warn('AgnesTextProvider: content is null, fallback to reasoning_content', {
                ...
                logger_1.logger.info('AgnesTextProvider: chatCompletion done', {
                ...
                if (err.message?.includes('429') || err.message?.includes('速率限制')) {
                    const waitMs = 5000 + attempt * 10000;
                    logger_1.logger.warn('AgnesTextProvider: retrying', { attempt: attempt + 1, waitMs });
                    await new Promise(r => setTimeout(r, waitMs));
                    continue;
                }
                throw err;
        ...
        logger_1.logger.error('AgnesTextProvider: all retries exhausted', { lastError: lastError?.message });
    ...
    async *streamChatCompletion(opts) {
        ...
        logger_1.logger.info('AgnesTextProvider: streamChatCompletion start', {
            messageCount: opts.messages.length,
            temperature,
            maxTokens,
            enableThinking,
        ...
        if (!response.ok) {
            clearTimeout(timeoutId);
            const errorText = await response.text();
            logger_1.logger.error('Agnes stream API error', { status: response.status, errorText });
            throw new Error(`Agnes API 错误 (${response.status}): ${errorText.slice(0, 200)}`);
        }
        if (!response.body) {
            clearTimeout(timeoutId);
                ...
                for (const event of events) {
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            logger_1.logger.warn('AgnesTextProvider: SSE chunk parse failed', { dataStr: dataStr.slice(0, 100) });
                            continue;
                        }
        ...
        logger_1.logger.info('AgnesTextProvider: streamChatCompletion done', { finalUsage });
    }
}
exports.AgnesTextProvider = AgnesTextProvider;
exports.agnesTextProvider = new AgnesTextProvider();
