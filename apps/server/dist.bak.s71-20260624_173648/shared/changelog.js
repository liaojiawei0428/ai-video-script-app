"use strict";
/**
 * apps/server/src/shared/changelog.ts
 *
 * v3.0.29 (S64): 版本 changelog 读取模块
 *
 * 之前 /api/version 硬编码 `changelog: '优化性能，修复已知问题'`, 用户
 * 访问 web 端 /download 页时看到的 changelog 是死字符串, 跟实际发布内容
 * 没关系 (BUG-066)。
 *
 * 修法: 维护一个 changelog.json 数据源, 按版本号维护真实条目, server 启动
 * 时一次性读入内存, /api/version 直接返回当前版本对应的真实 changelog。
 *
 * 数据源: `apps/server/changelog.json` (位于源码根, build 后随 dist 复制)
 *
 * 配套规范:
 * - VERSION_MANAGEMENT.md § 4 changelog 维护流程
 * - BUGS.md BUG-066 (硬编码 changelog)
 * - CODING_STANDARDS.md 第 30 条 (changelog 必真实)
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.readChangelog = readChangelog;
exports.listChangelog = listChangelog;
const fs_1 = require("fs");
const path_1 = require("path");
const DEFAULT_ENTRY = {
    version: '0.0.0',
    buildDate: '1970-01-01',
    summary: '初始版本',
    highlights: [],
    type: 'patch',
};
let _cache = null;
function loadChangelog() {
    if (_cache)
        return _cache;
    // 兼容两种路径: ts 源码 (src/) + tsc 编译后 (dist/)
    const candidates = [
        (0, path_1.join)(__dirname, '../../changelog.json'), // dist/shared/changelog.js → dist/changelog.json
        (0, path_1.join)(__dirname, '../changelog.json'), // src/shared/changelog.ts → src/changelog.json (tsx 跑)
        (0, path_1.join)(process.cwd(), 'changelog.json'), // pm2 启动时 cwd=/www/wwwroot/shipin-APP
    ];
    for (const path of candidates) {
        if ((0, fs_1.existsSync)(path)) {
            try {
                const raw = (0, fs_1.readFileSync)(path, 'utf-8');
                const data = JSON.parse(raw);
                const map = new Map();
                for (const e of data.entries ?? []) {
                    map.set(e.version, e);
                }
                _cache = map;
                console.log(`[changelog] loaded ${map.size} entries from ${path}`);
                return map;
            }
            catch (err) {
                console.warn(`[changelog] failed to parse ${path}:`, err);
            }
        }
    }
    console.warn('[changelog] no changelog.json found, using DEFAULT_ENTRY');
    _cache = new Map([[DEFAULT_ENTRY.version, DEFAULT_ENTRY]]);
    return _cache;
}
function readChangelog(version) {
    const map = loadChangelog();
    return (map.get(version) ?? {
        ...DEFAULT_ENTRY,
        version,
        summary: '本次更新优化性能，修复已知问题',
        highlights: [],
    });
}
function listChangelog() {
    const map = loadChangelog();
    return Array.from(map.values()).sort((a, b) => b.version.localeCompare(a.version, undefined, { numeric: true }));
}
