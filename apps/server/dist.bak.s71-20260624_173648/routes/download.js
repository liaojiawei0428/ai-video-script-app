...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    ...
    if (!ALLOWED_HOSTS.includes(parsed.hostname)) {
        logger_1.logger.warn('Download proxy: blocked non-whitelisted host', { host: parsed.hostname, userId: req.userId });
    ...
    try {
        logger_1.logger.info('Download proxy start', { url: url.slice(0, 100), filename: safeFilename, disposition, userId: req.userId });
        ...
        if (!upstream.ok && upstream.status !== 206) {
            logger_1.logger.error('Download proxy upstream error', { status: upstream.status, url });
        ...
        res.end();
        logger_1.logger.info('Download proxy done', { filename: safeFilename, disposition, totalBytes, userId: req.userId });
    }
    catch (err) {
        logger_1.logger.error('Download proxy error', { error: err.message, url });
        if (!res.headersSent) {
            res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: '下载失败: ' + err.message } });
        }
    }
});
