...
exports.STYLE_SYSTEM = void 0;
exports.inferComicStyle = inferComicStyle;
exports.extractShortIdentity = extractShortIdentity;
exports.selectMainReferenceCharacter = selectMainReferenceCharacter;
exports.buildCastBlock = buildCastBlock;
exports.comicGenerationSystemPrompt = comicGenerationSystemPrompt;
exports.comicGenerationUserPrompt = comicGenerationUserPrompt;
exports.calculateComicLayout = calculateComicLayout;
// ── 风格系统 (5 套预设) ──
exports.STYLE_SYSTEM = {
    realistic_cinematic: {
        zh: '电影写实风格',
 ...
 */
function inferComicStyle(styleBible, userOverride) {
    if (userOverride)
        return userOverride;
    if (!styleBible)
 ...
 */
function extractShortIdentity(sheetPrompt) {
    if (!sheetPrompt || !sheetPrompt.trim())
        return '';
    // 切到 --- 之前 (去掉 "character sheet, multiple views" 段)
    let text = sheetPrompt;
    const dashIdx = text.indexOf('---');
    if (dashIdx > 0)
        text = text.slice(0, dashIdx);
 ...
 */
function selectMainReferenceCharacter(characters) {
 ...
 */
function buildCastBlock(characters, mainRef) {
    if (characters.length === 0)
        return '';
 ...
 */
function comicGenerationSystemPrompt(style, layout, characters) {
    const styleDef = exports.STYLE_SYSTEM[style];
 ...
 */
function comicGenerationUserPrompt(input) {
 ...
 *   7+  → 3x3 (多页, 每页 9 个)
 */
function calculateComicLayout(shotCount) {
    if (shotCount <= 0)
        return { layout: '3x3', shotsPerPage: 9, totalPages: 0 };
