...
exports.findFirstMissingField = exports.SYSTEM_PROMPT = void 0;
exports.parseLLMDecisionV2 = parseLLMDecisionV2;
exports.countRequiredFilled = countRequiredFilled;
const imagePlanFields_1 = require("./imagePlanFields");
Object.defineProperty(exports, "findFirstMissingField", { enumerable: true, get: function () { return imagePlanFields_1.findFirstMissingField; } });
exports.SYSTEM_PROMPT = `你是"生图助手", 帮用户整理各种类型的 AI 生图需求 (人物/LOGO/风景/产品/概念图/其他)。
...
${Object.entries(imagePlanFields_1.SCENE_FIELD_HINTS).map(([scene, fields]) => {
    const requiredKeys = fields.filter(f => imagePlanFields_1.PLAN_FIELDS_META.find(m => m.key === f.key)?.required).map(f => f.label);
...
function parseLLMDecisionV2(raw, currentFields) {
    ...
    const result = { status: obj.status };
    if (obj.status === 'clarify') {
        if (typeof obj.missing_field === 'string')
            result.missing_field = obj.missing_field;
        if (typeof obj.question === 'string')
            result.question = obj.question;
        if (obj.partial_fields && typeof obj.partial_fields === 'object') {
            result.partial_fields = obj.partial_fields;
        }
    }
    else {
        // plan_cn_ready
        if (obj.plan_fields && typeof obj.plan_fields === 'object') {
            const pf = obj.plan_fields;
            const cleaned = {};
            ...
            for (const meta of imagePlanFields_1.PLAN_FIELDS_META) {
                if (typeof pf[meta.key] === 'string' && pf[meta.key].trim()) {
                    cleaned[meta.key] = pf[meta.key].trim();
                }
            }
...
function countRequiredFilled(fields) {
    if (!fields)
        return 0;
    let n = 0;
    // v3.0.0.8: 软必填只有 subject + style + quality
    const softRequired = ['subject', 'style', 'quality'];
