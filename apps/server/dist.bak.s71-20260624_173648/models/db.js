...
async function getDb() {
    ...
    try {
        ...
        logger_1.logger.error('MySQL connection failed', { error: err });
        throw err;
    }
    await initTables();
...
async function initTables() {
  ...
  `);
    // 兼容迁移：添加 INDEX
    try {
        await db.execute('ALTER TABLE task_jobs ADD INDEX idx_tasks_status (status)');
    }
    catch { }
    try {
        await db.execute('ALTER TABLE task_jobs ADD INDEX idx_tasks_novel (novel_id)');
    }
    catch { }
    // ======== 用户角色 ========
    try {
        await db.execute("ALTER TABLE users ADD COLUMN role VARCHAR(10) DEFAULT 'user'");
    }
    catch { }
    // ======== 充值申请记录 ========
    await db.execute(`
  ...
  `);
    ...
    try {
        await db.execute("ALTER TABLE users ADD COLUMN vip_level TINYINT DEFAULT 0");
    }
    catch { }
    // 添加 vip_expires_at 列（VIP 到期时间戳，毫秒）
    try {
        await db.execute("ALTER TABLE users ADD COLUMN vip_expires_at BIGINT DEFAULT NULL");
    }
    catch { }
    // 添加 last_ip 列（最近登录IP）
    try {
        await db.execute("ALTER TABLE users ADD COLUMN last_ip VARCHAR(45) DEFAULT ''");
    }
    catch { }
    // 添加 ip_location 列（IP归属地）
    try {
        await db.execute("ALTER TABLE users ADD COLUMN ip_location VARCHAR(100) DEFAULT ''");
    }
    catch { }
    // 通知/消息表
    await db.execute(`
  ...
  `);
    ...
    try {
        await db.execute("ALTER TABLE billing_logs ADD COLUMN is_free TINYINT(1) DEFAULT 0");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE billing_logs ADD COLUMN ref_type VARCHAR(50) DEFAULT ''");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE billing_logs ADD COLUMN ref_id VARCHAR(100) DEFAULT ''");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE billing_logs ADD COLUMN ref_label VARCHAR(200) DEFAULT ''");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE billing_logs ADD INDEX idx_billing_ref_type (ref_type)");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE billing_logs ADD INDEX idx_billing_user_time (user_id, created_at)");
    }
    catch { }
    // ════════════════════════════════════════════════════════════
    ...
    // ── characters: 加 8 字段 ──
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN description JSON DEFAULT NULL COMMENT '11维度结构化描述'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN extra_description JSON DEFAULT NULL COMMENT '4维度补充描述'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN style_id VARCHAR(36) DEFAULT 'realistic' COMMENT '画风ID'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN confirmed TINYINT(1) DEFAULT 0 COMMENT '用户是否已确认描述'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN image_variants JSON DEFAULT NULL COMMENT '3张变体图'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN image_gen_status VARCHAR(20) DEFAULT 'none' COMMENT 'none/generating/partial/completed/failed'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN confirmed_at BIGINT DEFAULT NULL");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE characters ADD COLUMN image_generated_at BIGINT DEFAULT NULL");
    }
    catch { }
    // ── novels: 加 5 字段 ──
    try {
        await db.execute("ALTER TABLE novels ADD COLUMN style_id VARCHAR(36) DEFAULT 'realistic' COMMENT '小说统一画风'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE novels ADD COLUMN plot_graph JSON DEFAULT NULL COMMENT '章节事件图谱'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE novels ADD COLUMN outline_confirmed TINYINT(1) DEFAULT 0");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE novels ADD COLUMN outline_confirmed_at BIGINT DEFAULT NULL");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE novels ADD COLUMN plot_graph_generated_at BIGINT DEFAULT NULL");
    }
    catch { }
    // ── episodes: 加 3 字段 ──
    try {
        await db.execute("ALTER TABLE episodes ADD COLUMN outline_text TEXT COMMENT '分集大纲'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE episodes ADD COLUMN confirmed TINYINT(1) DEFAULT 0");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE episodes ADD COLUMN character_descriptions JSON DEFAULT NULL COMMENT '生成时角色描述快照'");
    }
    catch { }
    // ── shots: 加 5 字段 ──
    try {
        await db.execute("ALTER TABLE shots ADD COLUMN image_url VARCHAR(500) DEFAULT ''");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE shots ADD COLUMN character_ids JSON DEFAULT NULL");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE shots ADD COLUMN style_id VARCHAR(36) DEFAULT NULL");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE shots ADD COLUMN image_prompt TEXT");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE shots ADD COLUMN image_generated_at BIGINT DEFAULT NULL");
    }
    catch { }
    // ── notifications: 加 2 字段 ──
    try {
        await db.execute("ALTER TABLE notifications ADD COLUMN priority VARCHAR(10) DEFAULT 'normal'");
    }
    catch { }
    try {
        await db.execute("ALTER TABLE notifications ADD COLUMN expires_at BIGINT DEFAULT NULL");
    }
    catch { }
    // ── 新表: assets 资产库 ──
    await db.execute(`
  ...
  `);
    ...
    `);
        ...
        logger_1.logger.warn('style_presets seed failed (可能已存在)', { error: err });
    }
    // ════════════════════════════════════════════════════════════
  ...
  `);
    ...
    try {
        await db.execute("ALTER TABLE video_conversations ADD COLUMN billing_status VARCHAR(20) DEFAULT 'settled'");
    }
    catch { }
    // ── 视频 Agent: 单次生成记录 (审计) ──
    await db.execute(`
  ...
  `);
    logger_1.logger.info('v3.0.0 Agent tables initialized (image_conversations, image_generations, video_conversations, video_generations)');
}
// 辅助函数：执行 SQL 并返回单行
