...
exports.shotModel = exports.ShotModel = void 0;
const db_1 = require("./db");
class ShotModel {
    async create(shot) {
        await (0, db_1.execute)(`INSERT INTO shots (id, episode_id, shot_number, scene_type, location, time_of_day,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [shot.id, shot.episodeId, shot.shotNumber, shot.sceneType, shot.location || '',
            ...
            shot.styleId || null,
            shot.imagePrompt || null]);
    }
    async findByEpisodeId(episodeId) {
        const rows = await (0, db_1.queryAll)('SELECT * FROM shots WHERE episode_id = ? ORDER BY shot_number', [episodeId]);
        return rows.map(row => this.mapRowToShot(row));
    }
    async findById(id) {
        const row = await (0, db_1.queryOne)('SELECT * FROM shots WHERE id = ?', [id]);
        if (!row)
            return undefined;
    ...
    async update(id, data) {
        ...
        if (data.shotNumber !== undefined) {
            fields.push('shot_number = ?');
            values.push(data.shotNumber);
        }
        if (data.sceneType !== undefined) {
            fields.push('scene_type = ?');
            values.push(data.sceneType);
        }
        ...
        if (data.cameraAngle !== undefined) {
            fields.push('camera_angle = ?');
            values.push(data.cameraAngle);
        }
        if (data.cameraMove !== undefined) {
            fields.push('camera_move = ?');
            values.push(data.cameraMove);
        }
        if (data.lighting !== undefined) {
            fields.push('lighting = ?');
            values.push(data.lighting);
        }
        if (data.durationSec !== undefined) {
            fields.push('duration_sec = ?');
            values.push(data.durationSec);
        }
        if (data.audioNote !== undefined) {
            fields.push('audio_note = ?');
            values.push(data.audioNote);
        }
        if (data.dialogue !== undefined) {
            fields.push('dialogue = ?');
            values.push(data.dialogue);
        }
        ...
        if (data.imageGeneratedAt !== undefined) {
            fields.push('image_generated_at = ?');
            values.push(data.imageGeneratedAt);
        }
        if (fields.length === 0)
            return;
        values.push(id);
        await (0, db_1.execute)(`UPDATE shots SET ${fields.join(', ')} WHERE id = ?`, values);
    }
    async bulkCreate(shots) {
...
exports.ShotModel = ShotModel;
exports.shotModel = new ShotModel();
