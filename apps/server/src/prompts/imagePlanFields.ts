...
// ── PlanFields 简化版 ──
// passthrough 模式只用 subject (用户原文) + negative (用户填的负向)
// 保留所有 10 字段 interface 是为了不破坏 API 兼容 (前端还在读 planFields.subject 等)
export interface PlanFields {
  scene_type?: string;        // 保留为可选, 不再主动使用
  subject: string;            // 用户原文 (passthrough 必用)
  action?: string;
  appearance?: string;
  expression?: string;
  environment?: string;
  ...
  style?: string;
  quality?: string;
  negative?: string;         // 用户填的负向 prompt
}

export const PLAN_FIELDS_META: Array<{
...
export function isCompletePlanFields(fields: any): fields is PlanFields {
  if (!fields || typeof fields !== 'object') return false;
  return typeof fields.subject === 'string' && fields.subject.trim().length > 0;
}

/** 找第一个缺失的必填字段 (passthrough 模式只查 subject) */
export function findFirstMissingField(fields: Partial<PlanFields> | null | undefined): keyof PlanFields | null {
...
export function getFilledFields(fields: Partial<PlanFields> | null | undefined): Array<{ key: keyof PlanFields; label: string; value: string }> {
  if (!fields) return [];
  return PLAN_FIELDS_META
...
export function fieldsToChineseDescription(fields: PlanFields): string {
  return (fields.subject || '').trim();
}

// ── SceneType 保留 (供 imageAspectRatio 等模块引用) ──
...
export const SCENE_FIELD_HINTS: Record<SceneType, Array<{ key: string; label: string }>> = {
