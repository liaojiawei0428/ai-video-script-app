...
export function buildI2VModificationPrompt(userText: string): string {
  return (userText || '').trim();
}
...
export function buildFinalEnglishPrompt(
  userText: string,
  userNegative: string = '',
): { enPrompt: string; negative: string } {
  const enPrompt = buildPassthroughPrompt(userText);
  const negative = buildNegativePrompt(userNegative);
  logger.info('imagePromptBuilder: passthrough 100%', {
...
export async function buildFinalEnglishPromptFromFields(fields: any): Promise<{ enPrompt: string; negative: string }> {
  const userText = (fields?.subject || '').toString();
  return buildFinalEnglishPrompt(userText, fields?.negative || '');
}

/** 兼容旧调用, 默认返回 true (不再做必填字段检查) */
export function isCompletePlanFields(_fields: any): boolean {
  return true;
}

/** 兼容旧调用, noop */
export async function expandChineseDescription(_fields: any): Promise<string> {
  return '';
}

/** 兼容旧调用, noop */
export async function translateToEnglishPrompt(_cnDescription: string): Promise<string> {
  return '';
}
