...
export const SCENE_TEMPLATES: Record<SceneType, PromptTemplate[]> = {
  character: PROMPT_TEMPLATES.filter(t => t.scene.includes('character')),
  logo:      PROMPT_TEMPLATES.filter(t => t.scene.includes('logo')),
  scene:     PROMPT_TEMPLATES.filter(t => t.scene.includes('scene')),
  product:   PROMPT_TEMPLATES.filter(t => t.scene.includes('product')),
  concept:   PROMPT_TEMPLATES.filter(t => t.scene.includes('concept')),
  other:     PROMPT_TEMPLATES.filter(t => t.scene.includes('other')),
};

export function selectBestTemplate(sceneType: SceneType, styleKeywords: string[]): PromptTemplate {
  const candidates = SCENE_TEMPLATES[sceneType] || SCENE_TEMPLATES.other;
  if (candidates.length === 0) {
    return PROMPT_TEMPLATES.find(t => t.id === 'sai-enhance')!;
  }
  ...
  for (const tpl of candidates) {
    ...
    for (const kw of styleKeywords) {
      ...
      if (!k) continue;
      if (tpl.name.toLowerCase().includes(k)) score += 3;
      for (const tag of tpl.tags) {
        const t = tag.toLowerCase();
        if (t === k || t.includes(k) || k.includes(t)) score += 1;
      }
    }
...
export function applyTemplate(tpl: PromptTemplate, userPrompt: string, userNegative: string = ''): { positive: string; negative: string } {
  return {
