...
export interface ParsedShot {
  shotNumber: number;        // 镜头编号 (从 1 开始)
  durationSec: number;       // 时长 (秒)
  sceneType: string;         // 景别: 大远景/远景/全景/中景/近景/特写/大特写/过肩镜头/航拍
  composition: string;       // 构图: 中心构图/三分法/对称构图/引导线/框架构图/黄金螺旋
  cameraMove: string;        // 运镜: 推/拉/摇/移/跟/升/降/环绕/固定/手持/航拍/斯坦尼康
  visual: string;            // 画面 (主体+动作+环境)
  dialogue: string;          // 对白 (含角色名+内容)
  dialogueLines: Array<{ character: string; line: string; emotion?: string }>; // 结构化对白
  action: string;            // 动作 (从画面提取)
  lighting: string;          // 灯光
  colorTone: string;         // 色彩调性
  audioNote: string;         // 音效/音乐
  transition: string;        // 转场
  imagePrompt: string;       // AI 生图 prompt
  rawText: string;           // 原始文本
  isHeader: boolean;         // 是否是开场白/元数据 (不是真正的镜头)
}
 ...
 * 解析单个镜头段落
 */
export function parseShotSegment(seg: string, index: number): ParsedShot {
  ...
  // 也支持直接匹配 【角色】"对白" 行
  const dialogueLines: ParsedShot['dialogueLines'] = [];
  let inDialogue = false;
  const allDialogueText: string[] = [];
  ...
  for (const line of lines) {
    if (isFieldLine(line)) {
      ...
      if (isDialogueLabel) {
        ...
        if (after) allDialogueText.push(after);
      } else {
        inDialogue = false;
      }
      continue;
    }
    if (line.match(/^---/)) { inDialogue = false; continue; }
    if (line.match(/^\[image_prompt\]/i)) { inDialogue = false; continue; }
    if (inDialogue) {
      if (line.trim()) allDialogueText.push(line.trim());
    }
  }
 ...
 * 判断段落是否是开场白/元数据
 */
export function isMetaParagraph(seg: string): boolean {
  const t = seg.trim();
  if (!t) return true;
  // 短段落 (无镜头标识)
  if (!t.match(/【?镜头\d+/)) return true;
  // 包含"好的"、"以下是"等开场白特征
  if (t.match(/^好的[，,。\s]/) || t.includes('以下是') || t.includes('开始生成')) return true;
  // 长度太短
 ...
 * 返回结构化分镜列表 (自动过滤开场白)
 */
export function parseShotList(rawText: string): ParsedShot[] {
  ...
  for (const seg of segments) {
    if (isMetaParagraph(seg)) continue;
    realIndex++;
 ...
 */
export function groupShotsByScene(shots: ParsedShot[]): Array<{ location: string; shots: ParsedShot[]; totalDuration: number }> {
