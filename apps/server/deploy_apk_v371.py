...
print(f'[1] scp APK → /www/wwwroot/shipin-APP/public/DeepScript_v3.0.71.apk ({local_size} bytes)')
out, err, rc = scp_file(LOCAL_APK, '/www/wwwroot/shipin-APP/public/DeepScript_v3.0.71.apk')
if rc != 0:
...
out, _, _ = ssh_run("sha256sum /www/wwwroot/shipin-APP/public/DeepScript_v3.0.71.apk")
print('  ' + out.strip())

# 3. 公网 HEAD 200 验证 (BUG-117 SOP 第 4 件套)
print()
print('[3] 公网 HEAD 验证 (https://ab.maque.uno/app/DeepScript_v3.0.71.apk)')
out, _, _ = ssh_run("curl -sIk -m 10 https://ab.maque.uno/app/DeepScript_v3.0.71.apk")
print(out)
...
out, _, _ = ssh_run(
    ...
    "echo \"10. APK HTTP/2 200:      $(curl -sIk -m 5 https://ab.maque.uno/app/DeepScript_v3.0.71.apk | head -1 | tr -d \\r)\"; "
    "echo \"10b. APK size:           $(curl -sIk -m 5 https://ab.maque.uno/app/DeepScript_v3.0.71.apk | grep -i 'content-length' | head -1 | tr -d \\r)\"; "
    "echo \"10c. APK sha256 (远端):   $(sha256sum /www/wwwroot/shipin-APP/public/DeepScript_v3.0.71.apk | awk '{print $1}')\""
)
print(out)
