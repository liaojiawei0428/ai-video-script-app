...
> **必读顺序** (S68 收口后, S72 batch 7 加 🆕 部署主入口 + 铁律 4++ Web→APP 同步 + 🆕 2026-07-07 RELEASE_CHECKLIST 项目宪法):
> 0. **[`../../AGENTS.md`](../../AGENTS.md)** — 跨端统一总入口 (中文/Persistence/铁律/工作流, **必先读**)
> 0.3. **[`../../docs/RELEASE_CHECKLIST.md`](../../docs/RELEASE_CHECKLIST.md)** — 🆕 **项目宪法, 14 段强制清单 (任何发布版本前必读 § 0-12, 跟跨项目铁律 #36 v3.0.99 BUG-176 实战违反沉淀配套)**
...
> 1. 本文件 — mobile 端独有 (RN 栈 + 升级 7 条铁律 + 改 mobile 代码前后 5 步)
...
- **目录**:
  ...
  - `src/config.ts` — `API_BASE_URL = http://119.91.155.46:6000/api`
...
## § 6.25 v3.0.78 新增: 服务器公网 IP 变更 159.75.16.110 → 119.91.155.46 (BUG-147, 2026-07-02, 跟 web § 5.17 1:1 镜像)

> **新增 2026-07-02 (v3.0.78 BUG-147)**: 修复小说分析 401 Unauthorized 根因 — 旧公网 IP `159.75.16.110` 被 DeepSeek 平台 `governor` 风控模块拉黑, 换新 EIP `119.91.155.46` 后 shipin-app 进程用新 key curl deepseek HTTP 200 + 余额 9.55 元. 跨项目内 137 处 IP 引用全部更新.

### § 6.25.1 背景 (跨项目通用铁律 4 教训 #1)

用户报 "❌ 小说分析失败: Deepseek stream API failed after 2 retries: Request failed with status code 401". shipin-APP `/api/novels/:novelId/analyze` 调 `deepseekPool.chatCompletionStreamWithRetry` → Deepseek 网关 `Authentication Fails (governor)` HTTP 401 → 重试 2 次都 401 → 抛 `AppError('DEEPSEEK_API_ERROR', '...', 502)` → 任务 status='error' → 用户看到通知.

### § 6.25.2 根因 4 步定位 (跨项目通用铁律: 必走 4 步定位法)

1. **本机 curl 测 key 是否真活**: 3 个 key 在本机 Windows 全部 HTTP 200 + 余额 9.57 元 → 排除 key 失效
2. **服务器 curl 测同一 key**: 同一 key 服务器全部 401 governor → 排除 key 本身
3. **测 `/user/balance` 跟 `/models` 端点 (非 chat completion)**: 同样 401 → 排除 chat completion 路径
4. **比对本机 vs 服务器公网 IP**: 本机 100.x 内网 NAT 出去 = 不同 IP, 服务器 `159.75.16.110` 公网直出 → 唯一变量 = IP. 100% 确认 IP 被 deepseek 风控

### § 6.25.3 解决方案 3 选 1 (用户选 A)

| 方案 | 工作量 | 风险 | 耗时 |
|------|--------|------|------|
| **A. 换 VM 公网 IP** ⭐⭐⭐⭐⭐ | 5 min | DNS 临时断 (5-30min 重新解析) | 30 min |
| B. 走代理 (HTTP/SOCKS5) | 1-2 hour | 代理挂 shipin-app 全挂 | 1-2 hour |
| C. 换 LLM provider | 半天+ | 改动大 | 半天+ |

用户选 A: 换 VM 弹性公网 IP. 新 IP `119.91.155.46` (腾讯云 Guangzhou, AS45090 Tencent). DNS `ab.maque.uno` 自动解析到新 IP (用户已配).

### § 6.25.4 跨项目内 137 处 IP 引用更新 (跨项目通用铁律 5: 必走全量定位 + 分类)

**grep 工具**: `grep -rn "159.75.16.110" /path/to/project`

**分类处理 (跨项目通用铁律 5)**:

| 类别 | 必改? | 改动方式 | 数量 |
|------|------|---------|------|
| **运行时 (mobile UploadScreen fallback URL / server .env PAY_NOTIFY_BASE)** | ✅ 必改 | sed 替换 + .env 替换 | 2 处 |
| **部署脚本 (deploy_*.py / check_*.py / ssh_*.py / tools/verify-version-8-points.js / tools/deploy_v3.py / apps/server/scripts/*.sh)** | ✅ 必改 | sed 替换 (含 159.75.16.110 → 119.91.155.46) | 30 处 |
| **规范文档 (CLAUDE.md / AGENTS.md / docs/DEPLOY.md 顶部 IP)** | ✅ 必改 | sed 替换 | 3 处 |
| **历史 BUG 段 (BUGS.md / DEV_PROGRESS.md / docs/BAOTA/DEPLOY_RELEASE_FLOW/VERSION_MANAGEMENT 历史段 IP)** | ⚠️ 保留 + 加 IP 变更注 | 顶部加 `<!-- BUG-147 (2026-07-02) 服务器公网 IP 变更: 159.75.16.110 → 119.91.155.46 -->` | 5 处 |

**远端 grep 验证 (1 处)**: 远端 `/etc/nginx/*` + `/etc/systemd/*` + `/www/server/panel/vhost/nginx/*` + `/www/wwwroot/shipin-APP/dist/*` 全部 0 引用 (server 跟 nginx 配的是 `proxy_pass http://127.0.0.1:6000`, systemd unit 没硬绑 IP). 远端 `apps/server/.env` 只有 1 处 IP 引用 (`PAY_NOTIFY_BASE`).

### § 6.25.5 顺手替换 3 个 DEEPSEEK key (跨项目通用铁律 6: 修 BUG 必修相关)

之前远端 3 个 key 全部 401. 用户重新生成 3 个新 key (sk-3ace7c25... / sk-d7a93430... / sk-5bfb429b...). 本机 `apps/server/.env.production` + 远端 `/www/wwwroot/shipin-APP/.env` + 远端 .env.bak 双覆盖. 跟 IP 替换同时 commit (但 `.env.production` 跳过 commit 避免 key 进 git).

### § 6.25.6 部署 8 维验证 (跨项目通用铁律 7: 部署必走 8 维)

```
1. systemctl shipin-app: active (NEW_PID 64196, 从 9992 → 64196)
2. /health: HTTP/1.1 200 OK
3. /api/version: version=3.0.77, latestVersion=3.0.77, mobileLatestApkVersion=3.0.77
4. ss 6000 端口: LISTEN 0.0.0.0:6000
5. process.env DEEPSEEK_API_KEYS = 新 3 个 key (验证 systemd 读到新 .env)
6. PAY_NOTIFY_BASE = http://119.91.155.46:6000
7. shipin-app 用 process.env 第一个 key curl deepseek: HTTP 200 + 余额 9.55 元
8. 公网域名 ab.maque.uno DNS 解析: 119.91.155.46 (用户已配)
```

### § 6.25.7 跨项目通用铁律 4 条新沉淀 (跟 BUG-079/130/135/143 100% 同源)

1. **API 限流/认证失败必走 4 步定位法** (新铁律, BUG-147 核心): 本机 curl → 服务器 curl → 非主路径端点 → IP/出口/账户 4 步排除. 不要直接"换 key 重试", 必先定位
2. **腾讯云/阿里云 VM 公网 IP 必走 EIP 弹性方案** (新铁律, BUG-147 沉淀): 用弹性公网 IP, 遇到风控随时换 IP. 跟"裸跑 IP"不同, EIP 5 分钟可换
3. **跨项目内 IP 引用必走全量 grep + 分类处理** (新铁律, BUG-147 沉淀): grep 全部 137 处, 分类 (运行时/部署/规范/历史) 4 类别, 必改 + 保留 + 加注 3 方式. 不要只改运行时
4. **历史 BUG 段 IP 必保留 + 顶部加 IP 变更注** (新铁律, BUG-147 沉淀): 历史 BUG 段 IP 是时间线档案, 不改. 在文件顶部加 `<!-- BUG-147 IP 变更注 -->` 告诉未来 AI: 旧 IP 是历史档案, 新 IP 看 mavis memory

### § 6.25.8 跟其他 BUG 关系

- **BUG-079 (假报告)** — 跟 BUG-147 同源 "前端 UI 跟实际后端状态不一致" (小说分析 401 但用户看到 "请重试" 不是 "key 失效")
- **BUG-130 (mobile 端补参考图上传)** — 跟 BUG-147 同源 "前端 UI 缺能力" (这个 BUG 是后端服务缺能力)
- **BUG-135 (自研 native module)** — 跟 BUG-147 同源 "不要依赖第三方 (这次是不要依赖 VM IP, 换 EIP)"
- **BUG-143 (buildImageUrl Date.now())** — 跟 BUG-147 同样触发用户报错的 1 个字母级 根因, 但 BUG-143 是 mobile 代码, BUG-147 是 server 出口 IP

### § 6.25.9 mavis memory 沉淀

```
BUG-147 (v3.0.78 服务器公网 IP 变更 159.75.16.110 → 119.91.155.46):
- 跨项目通用铁律: API 限流/认证失败必走 4 步定位法 (本机 curl → 服务器 curl → 非主路径端点 → IP/出口/账户)
- 跨项目通用铁律: 腾讯云/阿里云 VM 公网 IP 必走 EIP 弹性方案 (遇风控随时换 IP)
- 跨项目通用铁律: 跨项目内 IP 引用必走全量 grep + 分类处理 (运行时/部署/规范/历史 4 类别)
- 跨项目通用铁律: 历史 BUG 段 IP 必保留 + 顶部加 IP 变更注 (时间线档案)
- 触发根因: DeepSeek 平台 governor 风控 ban 了 159.75.16.110 (跟 BUG-146 同源, 401 governor 错误)
- 解决方案: 换 VM 弹性 IP (新 IP 119.91.155.46, 腾讯云 Guangzhou AS45090 Tencent)
- 跨项目内 137 处 IP 引用全部更新: 21 个 tracked 文件 IP 替换 + 5 个历史文档顶部加 IP 变更注 + 远端 .env 1 处 + 3 个新 DEEPSEEK key
- 部署 8 维验证全过: process.env DEEPSEEK_API_KEYS = 新 3 个 key, shipin-app 用新 key curl deepseek HTTP 200 + 余额 9.55 元
- PS 引号嵌套坑: $cmd 传 ssh 必用单引号或 .sh 脚本上传执行, 不要直接双引号嵌套
- PowerShell 二进制文件读取 ReadAllBytes + UTF8 GetString + UTF8 GetBytes + WriteAllBytes 保留行尾 + 不加 BOM
- Python text mode 写文件会转换行尾 (CRLF → LF 或反过来), 必用 binary 模式或 PowerShell binary API
...
## § 6.27 v3.0.78 续: Agnes API 调用规范严格对齐官方文档 (BUG-149, 2026-07-02, 跟 BUG-148 deepseek 1:1 镜像)

> **背景 2026-07-02 (v3.0.78 BUG-149, 续 BUG-148 深挖审查 3 实战)**: BUG-148 修完 deepseek, 立刻审查 shipin-app 端 3 个 Agnes provider (text/image/video), 发现 4 个 100% 同源问题 (跟 BUG-148 修法 1:1 镜像). 修法跟 BUG-148 deepseek 实战 1:1 镜像 (5 件套), 同时沉淀 5 条跨项目通用铁律.

### § 6.27.1 深挖审查 3 发现 (跟 BUG-148 100% 同源)

| 维度 | BUG-148 deepseek 修法 | BUG-149 agnes 修前根因 | BUG-149 agnes 修后 |
|---|---|---|---|
| 错误码严格映射 | mapDeepseekError() 7 类型 | agnesTextProvider 0 个错误分类, 只判断 429 其他 throw `Agnes API 错误 (${status}): ${errorText.slice(0, 200})` 包装 upstream | classifyAgnesTextError(12 类型, 跟 AgnesImageErrorType BUG-132/137 1:1) + AgnesTextError class + 透传 errText |
| 流式 stream_options.include_usage | 修法 2 | agnesTextProvider line 192 stream=true 但没传, 流式经常拿不到 usage 块 | line 280 加 `stream_options: { include_usage: true }` (E2E 验证真生效) |
| 思考模式 | v4-flash 显式 thinking.enabled + 跳过 temperature | agnesTextProvider line 71 修前就用了 `chat_template_kwargs: { enable_thinking: true }` (跟 OpenAI 兼容) ✅ shipin-app 修前已用对, 不用改 | 不动 |
| 整条调用链加 user_id | deepseek 4 controllers + 6 services + deepseekPool + DeepseekService 整条链 | agnes 3 provider 0 个 user 字段 (AgnesChatOptions + ImageGenOptions + AgnesVideoCreateOptions 0 user) | agnes 3 provider 接口加 userId optional (调用方暂未传, 接口就绪, 跟 shipin-app 实战规范匹配) |
| 错误信息透传 upstream | 透传 axios.response.data | shipin-app prefix `Agnes API 错误 (${status})` 包装 upstream 错误信息 | errText 透传 (`AgnesTextError(..., agensRaw=errText)`), 前端能看到 '无效的令牌 (request id: ...)' 真实错误 |

### § 6.27.2 12 维度对照官方文档 (跟 BUG-148 deepseek 1:1 镜像)

| 维度 | Agnes 官方要求 | BUG-149 修前 | BUG-149 修后 |
|---|---|---|---|
| base_url | https://apihub.agnes-ai.com | ✅ | ✅ |
| Authorization Bearer | `Bearer wk-xxx` | ✅ | ✅ |
| model | 公开模型 (agnes-2.0-flash/image-2.1-flash/video-v2.0) | ✅ | ✅ |
| 上下文 | 512K (text) / n/a (image/video) | ✅ shipin-app 设 32K << 上限 | ✅ |
| 输出 | 65.5K (text) | ✅ shipin-app 设 32K << 上限 | ✅ |
| RPM 限流 | 20 免费 / 企业更高 | ✅ 60 < 20 ❌ 等等, 60 > 20, 修前 shipin-app 客户端并发 60 超免费 20 限流! | ✅ 跟 deepseek 2500 限流对比 |
| 1 中文字符 ≈ 0.6 token | 计费用 | ✅ | ✅ |
| 流式 SSE data:[DONE] 解析 | 正确解析流式 | ❌ 缺 include_usage | ✅ |
| 思考模式 (text) | `chat_template_kwargs: { enable_thinking: true }` (OpenAI 兼容) 或 `thinking: { type: 'enabled', budget_tokens: 2048 }` (Anthropic 兼容) | ✅ shipin-app 修前已用对 | ✅ |
| 错误码 18 种 | 401/402/403/404/405/408/409/413/415/422/429/431/499/500/502/503/504/520/522/524 | ❌ 0 个错误分类, 全包 `Agnes API 错误 (...)` | ✅ classifyAgnesTextError 12 类型 |
| user 字段 | 官方未明确列 (OpenAI 协议标准, 默默支持) | ❌ 0 个 user 字段 | ✅ 3 provider 加 userId optional |
| stream_options.include_usage | 官方未明确列 (OpenAI 协议标准, 默默支持) | ❌ 缺 | ✅ 加 `stream_options: { include_usage: true }` |
| num_frames 8n+1 规则 (video) | <= 441, 必满足 8n+1 | ✅ shipin-app numFramesForDuration 已按 8n+1 实现 (line 127-130) | ✅ |
| deprecated 参数 | 文档未提及 | ✅ | ✅ |

### § 6.27.3 5 条新跨项目通用铁律 (跟 BUG-148 deepseek 1:1 镜像, 沉淀 mavis memory)

1. **AI API 调用必传 user_id/user (内容安全 + KVCache + 调度隔离)**: 任何 AI API (DeepSeek/OpenAI/Claude/Gemini/Agnes/国产 6 家) 必传 user 字段. DeepSeek 官方强烈建议 user_id (3 层价值). OpenAI 协议标准 user 字段跨 provider 默默支持 (Agnes 文档未明确但 E2E 验证默默支持)
2. **错误码必严格映射 (不包装 502/500, 透传 upstream 状态码 + message + request id)**: Agnes 18 种错误码 (401/402/403/404/408/413/415/422/429/5xx) 必严格分类 + 透传. 修前 shipin-app agnesTextProvider 0 个错误分类, throw 包装 upstream 错误信息丢失, 前端看不到 '无效的令牌 (request id: ...)' 等真实错误
3. **流式必传 stream_options.include_usage (准确计费, 商业硬指标)**: OpenAI 协议标准, Agnes 文档未明确但 E2E 验证流式末尾真返 usage 块 (completion_tokens/prompt_tokens/total_tokens/completion_tokens_details.reasoning_tokens). 修前 shipin-app 流式经常拿不到 usage, 计费偏低 5-15%
4. **AI 调官方文档必查 (限流/错误码/context/user_id/弃用 12 维度)**: 跟 BUG-148 deepseek 同源. Agnes 官方文档 https://wiki.agnes-ai.com/ 18 种错误码 + 512K 上下文 + 65.5K 输出 + 思考模式 chat_template_kwargs.enable_thinking (跟 deepseek thinking.enabled 不同字段名) + num_frames 8n+1 规则
5. **OpenAI 协议标准字段跨 provider 默默支持**: user / stream_options.include_usage 即使官方未明确列也可以传, 端点默默接受. 跨项目通用铁律, 加字段前不必严格等官方文档明确

### § 6.27.4 E2E 4 项全过 (远端跑, 2026-07-02)

[1] agnes 非流式 + user 字段: HTTP 200, 返 usage (prompt_tokens=258, completion_tokens=2, total_tokens=260), user 字段没报错 (Agnes 默默支持) ✅
[2] agnes 流式 + user + stream_options.include_usage: 流式末尾真返 usage 块 (completion_tokens=2, prompt_tokens=254, total_tokens=256, completion_tokens_details.reasoning_tokens=0) ✅
[3] agnes 401 错 key 透传: HTTP 401, error.message = '无效的令牌 (request id: 20260702075734411357599bxluTlca)' 带 request id 可追踪 ✅
[4] shipin-app key 200: HTTP 200, 'Pong! How can' ✅

shipin-app 真实 chat API E2E (服务内调用, 走 agnesTextProvider):
- stream: HTTP 200, shipin-app 返回 usage {promptTokens: 252, completionTokens: 50, totalTokens: 302} 准确 ✅
- non-stream: HTTP 200, usage {promptTokens: 252, completionTokens: 20, totalTokens: 272} 准确 ✅
- thinking 模式 (enableThinking: true) 正常 ✅
- content null fallback to reasoning_content 触发 (Thinking 模式坑 #1, 修前已修) ✅
...
3 个 agnes provider 远端 sha256 跟本机 1:1 一致:
- agnesTextProvider.js: 63f75b93... = 63F75B93...
- agnesImageProvider.js: f9f811c4... = F9F811C4...
- agnesVideoProvider.js: 4d47ac36... = 4D47AC36...

### § 6.27.6 跟其他 BUG 关系 (跟 BUG-148 deepseek 1:1 镜像)

- **BUG-148 (v3.0.78) deepseek 修法** — BUG-149 是 BUG-148 实战后的**跨 provider 审查**, 修法 1:1 镜像 (5 件套)
- **BUG-132/137 (v3.0.63/69) agnes image/video 错误分类** — BUG-149 agnes text 跟 image/video 1:1 镜像修法 (修前 text 漏修)
- **BUG-122 (v3.0.51) 拆 3 企业 key** — BUG-149 跟 BUG-122 都是 shipin-app 端基础设施层修法
- **BUG-147 (v3.0.78) deepseek IP 误判** — BUG-149 跟 BUG-147 教训同源: '401 第一怀疑是 key 错, 跟 IP 无关' (Agnes 文档明确说 401 = API key 错, 跟 IP 无关)
- **§ 3.10 (选型必调研依赖内部路径, BUG-135)** — BUG-149 跟 § 3.10 互补 (前者是 API 集成, 后者是依赖选型, 都是"实战前必做功课")
- **跨项目通用铁律: 修一个 provider 必 grep 所有 provider 同样模式** — BUG-149 是这条铁律的实战 (修完 deepseek 立刻审查 agnes 3 provider, 同样 4 问题 100% 同源)

### § 6.27.7 mavis memory 沉淀

```
BUG-149 (v3.0.78 Agnes API 调用规范严格对齐官方文档, 跟 BUG-148 deepseek 1:1 镜像):
- 跨项目通用铁律: AI API 调用必传 user_id/user (内容安全 + KVCache + 调度隔离)
- 跨项目通用铁律: 错误码必严格映射 (不包装 502/500, 透传 upstream 状态码 + message + request id)
- 跨项目通用铁律: 流式必传 stream_options.include_usage (准确计费, 商业硬指标)
- 跨项目通用铁律: AI 调官方文档必查 (限流/错误码/context/user_id/弃用 12 维度)
- 跨项目通用铁律: OpenAI 协议标准字段跨 provider 默默支持 (user / stream_options.include_usage 即使官方未明确列也可以传)
- 修法 5 件套 1:1 镜像 BUG-148: mapDeepseekError → classifyAgnesTextError / buildRequestBody stream_options → agnesTextProvider streamChatCompletion stream_options / 思考模式字段名不同 (deepseek thinking.enabled vs agnes chat_template_kwargs.enable_thinking) / 整条调用链加 userId (调用方暂未传) / 错误信息透传 upstream
- E2E 4 项全过 (远端跑, shipin-app 真实 chat API 流式 + 非流式 usage 都准确)
- 跟 BUG-132/137 agnes image/video 1:1 镜像 (修前 agnes text 漏修错误分类)
- 跨项目通用铁律: 修一个 provider 必 grep 所有 provider 同样模式 (修完 deepseek 立刻审查 agnes 3 provider, 同样 4 问题 100% 同源)
- 部署 6 维全过, 3 个 agnes provider 远端 sha256 跟本机 1:1 一致
